...
> 업데이트: `M=128`의 Gray 결과는 별도 `200` rollout 재평가에서 `31/200 = 15.5%`로 재확인됐다.
> 통계 주의: 나머지 표는 n_rollouts=50, single seed. 95% 구간 약 ±8~10%p.
...
> 특히 M=128에서 2%→15.5%, M=1024에서 2%→18%로 개선됐다.
...
| λ | success rate | successes | best_loss | 해석 |
|---|---:|---:|---:|---|
| 0.0 | 2% | 1/50 | 1.5710 | 직교성 제약 없음 |
| 0.001 | 8% | 4/50 | 1.6125 | 약한 regularization |
| 0.01 | 12% | 6/50 | 1.6275 | 현재 default |
| 0.1 | 12% | 6/50 | 1.6233 | default와 동일 |
| 1.0 | 16% | 8/50 | 1.6191 | 추가 개선 |
| 5.0 | **24%** | **12/50** | 1.6176 | 이 ablation의 best |
| 10.0 | 10% | 5/50 | 1.6123 | 너무 강하면 성능 하락 |
...
k개 비트가 전부 동일한 정보를 인코딩하게 되어 `2^k`개 셀이 아닌 사실상 극히 적은 수의 셀만 쓰게 된다.
