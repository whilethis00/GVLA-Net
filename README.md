...
이 접근은 단순히 속도를 높이는 것을 넘어, 로봇 제어에서 오래된 **상충 관계(Trade-off)를 실질적으로 완화**합니다:

- 기존: 정밀한 행동 공간(큰 N) = 느린 추론 = 실시간 어려움
- GVLA-Net: 정밀한 행동 공간(큰 N) = **추론 비용이 N에 거의 의존하지 않음** = 고해상도 행동 공간에서도 저지연 추론 가능
...
### 1. 스케일링 법칙: 행동 공간 크기에 따른 추론 비용 분석

행동 공간 크기 N을 10,000에서 1,000,000까지 늘리며 행동 선택 헤드의 레이턴시를 측정했습니다 (합성 벤치마크, GPU A100, 단일 쿼리 기준).

**GVLA-Net의 헤드 레이턴시는 N이 증가해도 거의 일정한 반면, Softmax 헤드는 선형으로 증가합니다.**
...
**결과 해석**: 합성 벤치마크 기준, N ≈ 5만 이상부터 GVLA-Net 헤드가 Softmax 헤드보다 빠르게 측정되며, N = 100만에서 약 20배의 헤드 레이턴시 차이가 관찰됩니다. GVLA-Net의 헤드 레이턴시는 ~0.16ms로 거의 일정하게 유지되는데, 이는 `k = ⌈log₂N⌉` 차원의 선형 투영만 수행하기 때문입니다. 단, 이 수치는 헤드 단독 측정이며 백본 추론 비용은 포함되지 않습니다.
...
**결과 해석**: N=1k처럼 작은 행동 공간에서는 기존 헤드 대비 이점이 제한적이며(pi0.5의 경우 소폭 느림), 이 구간에서 GVLA-Net이 우위에 있다고 보기 어렵습니다. N이 32k 이상으로 커지면 헤드 레이턴시 기준으로 50× 이상의 차이가 측정됩니다. 이 결과는 **헤드 단독 레이턴시** 비교이며, 실제 end-to-end 태스크 성능이나 백본 포함 전체 시스템 속도와는 별개입니다.
...
**결과 해석**: 이 단순화된 시뮬레이션 환경에서 GVLA-Net이 약 23배 낮은 추적 오차를 보입니다. 이 환경은 2자유도 2D 연속 추적으로 설계되어 기하학적 해싱이 유리한 조건입니다. 대규모 N에서 밀집 Softmax는 logit 차이가 작아져 구별력이 떨어지는 경향이 있으며, 기하학적 해싱은 각 행동에 이진 코드를 부여해 이 문제를 완화합니다. 단, 이 결과를 다양한 실제 로봇 과제로 직접 일반화하려면 추가 검증이 필요합니다.
...
**결과 해석**: 직교성 훼손(σ=0.2) 시 정확도가 100%에서 11%로 급격히 감소하는 것이 관찰됩니다. 이는 직교 기저의 이론적 특성과 일관적인 결과입니다 — 행들이 직교성을 잃으면 서로 다른 투영이 유사한 방향을 가리키게 되고, 비트 독립성이 저하되어 유효 코드 수가 감소합니다.

이 실험 설정에서 직교성 손실이 성능에 상당한 영향을 미침을 보여주며, `L_ortho = ||WW^T - I||²_F` 정규화 항의 역할을 경험적으로 뒷받침합니다.
...
**결과 해석**:
두 조건의 success rate는 동일(0.100)하며, 이 숫자 자체가 task competence를 반영하지는 않습니다. `octo-base-1.5`는 이 경량 2D 환경이 아닌 실제 로봇 데이터로 학습된 정책이며, 현재 환경은 관측 분포·행동 의미·태스크 구조가 원래 학습 분포와 다른 OOD 설정입니다. 따라서 **이 롤아웃은 통합 파이프라인(adapter → action → env)이 올바르게 연결되어 있음을 검증하는 sanity check** 로 해석하는 것이 적절합니다.

주목할 지표는 **inference time**입니다. 4096 bins(12비트 해상도)는 256 bins보다 16배 정밀한 행동 공간임에도 추론 속도가 28% 빠르게 측정됩니다. mean_final_distance도 소폭 개선(0.8336 → 0.8074)되는데, 이는 finer quantization step(0.000244 vs 0.00392)이 목표 위치 접근 정밀도에 영향을 준 것으로 볼 수 있습니다. 단, 10회 롤아웃 기준이므로 통계적 유의성은 제한적입니다.
...
where $z \in \mathbb{R}^d$ is a **whitened isotropic Gaussian** latent variable ($z \sim \mathcal{N}(0, I_d)$) and the rows of $W \in \mathbb{R}^{k \times d}$ are unit norm. Under this isotropic assumption, when the rows of $W$ are mutually orthogonal, the projection bits are independent and balanced, giving

$$
H(Y) = \sum_{i=1}^{k} H(Y_i) = k.
$$

This maximizes effective code capacity under the stated distributional assumption. When the latent distribution deviates from isotropy, the independence argument no longer holds exactly, and the effective entropy may be lower.
...
Hence, under this isotropic Gaussian model, the codebook uses all $2^k$ orthants with equal probability, the joint entropy is

$$
H(b)=k,
$$

and any action set of size $N \le 2^k$ can in principle be assigned distinct codes. Equivalently, the minimum number of binary routing decisions needed to distinguish $N$ actions is

$$
k \ge \lceil \log_2 N \rceil.
$$

This is the sense in which orthogonality supports logarithmic decoding under an isotropic latent model: orthogonal projections decorrelate the bit outputs and push $H(b)$ toward the $k$-bit limit. Without orthogonality, $WW^\top \neq I_k$, so the projected coordinates become correlated and $H(b) < k$ in general, reducing the effective number of distinguishable codes. In practice, latent distributions are not perfectly isotropic, so this analysis provides a useful idealized bound rather than a guarantee.
...
로보틱스 분야에서 행동 공간이 정밀해질수록 추론 비용이 선형으로 증가한다는 것은 Softmax 기반 헤드의 구조적 한계였습니다. GVLA-Net은 이 병목을 헤드 설계 수준에서 실질적으로 완화하는 방향을 제시합니다.

- 20비트 행동 공간(100만 개)에서 GVLA 헤드의 레이턴시는 10비트(1천 개)와 합성 벤치마크 기준 거의 동일합니다.
- 헤드 단독 기준으로 7B VLA에서 >1000 FPS의 선택 주파수가 측정됩니다 (백본 추론 제외).
- 기하학적 접근법은 **백본 무관(backbone-agnostic)** — 잠재 벡터를 출력하는 VLA라면 헤드만 교체하는 방식으로 적용 가능합니다.
