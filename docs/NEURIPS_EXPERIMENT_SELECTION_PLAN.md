...
여기서 양자역학의 직교 측정은 물리적 등가가 아니라, 계산 구조를 설명하는 직관으로만 쓰는 것이 안전하다.

dense softmax는 각 action dimension에 대해 `M`개의 후보를 직접 모두 점수화한다.  
hidden dimension을 `h`, action dimension을 `d`라고 하면 출력 계산은 대략

`O(d h M)`

이다.

반면 GVLA는 하나의 거대한 `M`-way decision을 길이

`k = ceil(log2 M)`

인 binary routing으로 분해한다.  
이에 따라 head compute는
...
직관적으로는, 각 bit decision이 action space에 대한 하나의 binary measurement처럼 작동한다.  
여러 bit decision을 조합하면 `M`개의 후보를 직접 모두 비교하지 않고도 action code를 복원할 수 있다.

이 관점이 양자역학의 orthogonal measurement intuition과 닮아 있는 이유는, 큰 상태 공간을 하나의 거대한 전역적 스캔으로 처리하는 대신 여러 개의 binary measurement axis로 분해해 구분한다는 구조적 유사성 때문이다.

다만 여기서 중요한 것은 “양자역학과 똑같다”는 주장이 아니다.  
보다 정확히는, GVLA가 **orthogonal binary decomposition**에 가까운 구조를 사용해 large output space를 더 tractable하게 분해한다는 점이 핵심이다.
...
여기서 orthogonality와 Gray coding의 역할은 분리해서 설명해야 한다.

- orthogonality regularization은 projection axis들 사이의 중복 정보를 줄이도록 유도한다.
- Gray coding은 action-space locality를 code-space locality와 맞추는 역할을 한다.

즉 orthogonality는 binary measurement axis의 분해 품질에 관한 이야기이고, Gray code는 target code geometry의 locality mismatch를 줄이는 장치다.

이 구분이 중요하다. Gray code가 projection axis의 직교성을 “유지”하는 것은 아니며, 대신 인접 action이 code-space에서도 작은 perturbation으로 보이게 만들어 high-resolution regime에서 bit-wise BCE 학습 신호를 더 일관적으로 만든다.
