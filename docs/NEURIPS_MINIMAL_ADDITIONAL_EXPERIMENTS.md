...
아래 네 가지가 사실상 제출 전 필수다.

## 필수 1. `custom2.5` 고표본 재검증

### 왜 필요한가

현재 `custom2.5` 결과는 방향은 매우 좋다.

- `256` 실패
- `512`도 부족
- `1024~2048` 회복

하지만 `10 rollouts`는 논문 claim용으로는 약하다.  
high-resolution regime의 존재를 메인 결과로 쓰려면 최소 `100~200 rollouts` 수준의 신뢰도가 필요하다.
...
- `precision_custom25_200roll_core.json` 실행 중 또는 대기 결과 확인 필요
...
1. `custom2.5` `100~200 rollouts` 완료 및 CI 계산
2. Gray code main ablation 표 정리
3. latency/scaling 그림 정리
4. synthetic geometry 실험 추가
5. 제목/초록/서론을 robotics-first에서 scalable structured outputs-first로 수정
