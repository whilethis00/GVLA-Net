...
이건 high-resolution GVLA 성능 병목의 일부가 구조 자체가 아니라 `target coding mismatch`였다는 강한 증거다.
...
- Gray code main ablation (`128 / 1024`): 완료
- scaling / latency / memory figure: 완료
- synthetic geometry figure: 완료
- Gray code `2048` row: optional
...
`2048`이 없더라도 메인 본문은 `128`과 `1024`만으로 충분히 구성 가능하다.  
핵심은 범위를 넓히는 것이 아니라, **coding effect가 분명한 두 점을 깨끗하게 제시하는 것**이다.
...
- `2048`은 체크포인트는 있으나 `eval_results.json`에는 아직 없음

### 2048 row를 추가하려면

새 학습은 필요 없다.  
이미 있는 체크포인트를 rollout evaluation만 하면 된다.

사용할 체크포인트:

- `experiments/results/bc_study/checkpoints/dense_2048/`
- `experiments/results/bc_study/checkpoints/gvla_2048/`
- `experiments/results/bc_study/checkpoints/gvla_gray_2048/`

실무적으로는:

1. `bc_eval.py`로 위 세 모델을 평가한다.
2. 결과를 별도 JSON으로 저장한다.
3. 메인 표 또는 figure에 `2048` row를 추가한다.

다만 메인 claim을 위해 필수는 아니다.
